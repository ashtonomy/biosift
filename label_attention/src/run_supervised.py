...
sys.path.append(os.path.dirname(os.path.dirname(this_file)))
from utils.modeling_label_attention_bert import BertForLabelAttendedSequenceClassification
...
def main():
    ...
    model = BertForLabelAttendedSequenceClassification.from_pretrained(
    ...
    if training_args.do_train:
        ...
        # trainer.import_metrics("train", metrics)
        trainer.save_state()
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if labels is not None:
                ...
                try:
                    logger.info("Saving hidden states for test set.")
                    pooled_output = model(**predict_dataset)[1]
                    torch.save(pooled_output.detach().clone().cpu(), os.path.join(training_args.output_dir, "test_hidden_states.pt"))
                except:
                    logger.info("Unable to save hidden states.")
