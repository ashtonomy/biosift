...
class DataTrainingArguments:
    ...
    premise_column_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "Name of the column in the dataset containing the premise statement."
        },    )
    hypothesis_column_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "Name of the column in the dataset containing the hypothesis statement."
        },
    )
...
def main():
    ...
    def preprocess_function(examples):
        result = tokenizer(
            examples[data_args.premise_column_name],
            examples[data_args.hypothesis_column_name],
            padding=padding,
        ...
        if "PMID" in examples:
            result["PMID"] = examples["PMID"]
        if "Positive" in examples:
            result["Positive"] = examples["Positive"]
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        result = {}
        result["f1"] = f1_metric.compute(predictions=preds, references=p.label_ids)["f1"]
        result["precision"] = prec_metric.compute(predictions=preds, references=p.label_ids)["precision"]
        result["recall"] = rec_metric.compute(predictions=preds, references=p.label_ids,)["recall"]
